...
function ProductList({ user = null, onLogout = null, showFilters = true }) {
  ...
  // ТОЛЬКО ОДИН useEffect для загрузки данных
  useEffect(() => {
    ...
    const loadProducts = async () => {
      try {
        ...
        if (window.api && typeof window.api.getProducts === 'function') {
          ...
          // ОДНОВРЕМЕННО устанавливаем оба состояния
          setProducts(productsData || []);
          setLoading(false); // Сразу после установки продуктов
        } else {
          console.warn('❌ API not available');
          throw new Error('API не доступно');
        }
        ...
        console.error('❌ Error loading products:', error);
        // ОДНОВРЕМЕННО устанавливаем оба состояния при ошибке
        setError(error.message);
        setProducts([]);
        setLoading(false);
      }
